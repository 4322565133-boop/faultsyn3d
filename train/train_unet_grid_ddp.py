"""Paired U-Net-L / U-Net-L+grid training; epoch-boundary resumable, four-GPU DDP.
...
def validate(module, loader, device):
    ...
    for k, c in enumerate(CATEGORIES):
        result.update({f'{metric}_{c}': value for metric, value in score(stats[5+3*k:8+3*k]).items()})
    return result
...
def main():
    ...
    p.add_argument('--variant', choices=['baseline', 'grid'], required=True)
    ...
    mkw = dict(base=42, grid_mid=a.variant == 'grid', grid_dec=a.variant == 'grid', checkpoint_highres=True)
    module = build('unet_grid3d', **mkw).to(device)
    ...
    sources = ['models/unet_grid3d.py', 'models/unet3d.py', 'train/train_unet_grid_ddp.py',
               'train/dataset.py', 'train/train_old_recipe.py']
    config = {**{k:v for k,v in vars(a).items() if k not in ('resume', 'stop_after_epoch')},
              'model': 'unet_grid3d', 'mkw': mkw, 'params': sum(x.numel() for x in module.parameters()),
              ...
              'norm': 'Unchanged U-Net InstanceNorm (no affine/running stats); LayerNorm in added grid attention',
              ...
              'reference':'runs/maxvit3d_unet_l_ddp; paired control in same experiment root', 'data_source_sha256':train.base.source_sha256}
    ...
    fields = ['epoch','train_loss','train_seg','val_loss','val_iou','val_dice','val_precision','val_recall','lr','seconds',
              'updates','skipped_updates','amp_scale','peak_memory_mib']+[f'val_{metric}_{c}' for c in CATEGORIES for metric in ['iou','precision','recall']]
    ...
    if rank==0:
        ...
        print(f'UNet-L {a.variant}: {config["params"]:,} parameters; global batch {config["global_batch"]}; start epoch {start}', flush=True)
    epoch = start - 1
    for epoch in range(start, a.epochs+1):
        ...
        for step,(x,y,_) in enumerate(dl):
            ...
            if rank==0 and (step==0 or (step+1)%10==0):
                ...
                print(f'epoch={epoch:03d} step={step+1}/{len(dl)} loss={float(loss.detach()):.5f}',flush=True)
        ...
        if rank==0:
            ...
            row.update({f'val_{metric}_{c}':v[f'{metric}_{c}'] for c in CATEGORIES for metric in ['iou','precision','recall']})
